#!/bin/bash
# WHOOP CLI wrapper — pull health data from WHOOP API
# Usage: bash whoop.sh [recovery|sleep|cycle|profile|body|summary]
...
if [[ ! -f "$TOKEN_FILE" ]]; then
  echo "ERROR: No WHOOP tokens found. Run whoop-oauth.py first."
  exit 1
fi
...
if (( NOW - OBTAINED_AT > EXPIRES_IN - 120 )); then
  ...
  bash "$SCRIPT_DIR/whoop-refresh.sh" >/dev/null 2>&1 || {
    echo "ERROR: Token expired and refresh failed. Re-run whoop-oauth.py"
    exit 1
  }
fi
...
case "${1:-summary}" in
  ...
  body)
    ...
    ;;
  summary)
    echo "📊 WHOOP Summary — $(date '+%Y-%m-%d %I:%M %p ET')"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
"
    ;;
  *)
    echo "Usage: whoop.sh [recovery|sleep|cycle|profile|body|summary]"
    ;;
esac
