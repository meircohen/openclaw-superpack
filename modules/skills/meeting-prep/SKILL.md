...
- `calendar_account` — Which gog account to use for calendar (defaults to primary_email if not set)
...
- Calendar: Use {user.calendar_account} (from config) with gog
- **VALIDATION REQUIRED:** Before sending any meeting prep, verify calendar access with: `gog calendar list --account {calendar_account} --from TODAY --to TODAY`
- If calendar access fails, STOP and report the error - never send meeting prep based on assumptions
...
## Fireflies Integration (Added 2026-03-04)

### Pre-Meeting Context from Fireflies

Before each external meeting, pull prior meeting context if available:

```bash
source ~/.openclaw/.api-keys

# 1. Search for previous meetings with this person
PERSON_NAME="[attendee name from calendar]"

curl -s -X POST 'https://api.fireflies.ai/graphql' \
  -H "Authorization: Bearer $FIREFLIES_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"query": "query { transcripts(limit: 50) { id title date } }"}' | \
  jq -r ".data.transcripts[] | select(.title | contains(\"$PERSON_NAME\"))" | \
  sort -r | head -3

# 2. If found, fetch most recent meeting
LAST_MEETING_ID="[from above]"

curl -s -X POST 'https://api.fireflies.ai/graphql' \
  -H "Authorization: Bearer $FIREFLIES_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"query\": \"query { transcript(id: \\\"$LAST_MEETING_ID\\\") { title date sentences { speaker_name text } } }\"}"

# 3. Extract key topics and action items
jq -r '.data.transcript.sentences[] | "\(.speaker_name): \(.text)"' | \
  grep -i "action\|follow up\|next step\|decision\|commit"
```

### Enhanced Meeting Brief Format

Add Fireflies section to meeting prep output:

```
**Previous Meetings (Fireflies):**
- Last met: [Date] — [Meeting title]
- Key topics: [extracted topics]
- Action items from last call:
  • [Action 1] — Status: [completed/pending]
  • [Action 2] — Status: [completed/pending]
- Open questions: [unresolved items]

📎 Last meeting: https://app.fireflies.ai/view/[id]
```

### Integration Workflow

For each attendee in today's meeting:
1. **Check Fireflies:** Search by name in meeting titles
2. **If found:** Pull most recent 1-2 meetings
3. **Extract context:**
   - What was discussed
   - What was decided
   - What action items were assigned
   - What's still pending
4. **Add to brief:** Include in "Context" or new "Previous Meetings" section

### Fallback Priority
1. **Fireflies** (full transcript) — most detailed
2. **Granola** (AI summary) — good overview
3. **Email** (thread history) — relationship context
4. **LinkedIn** (if first meeting) — background only

Use Fireflies when available, fall back to Granola/email if no Fireflies recording.

### Example Enhanced Brief

```
*1. George Halachev — 12:00 PM*

**Who:** Executive coach specializing in habit formation and time management

**Previous meetings (Fireflies):**
- Feb 27, 2026: Discussed morning routine structure (Gym → Shower → Prayer → Task hour)
- Feb 20, 2026: Addressed overwhelm from unstructured days
- Action item status:
  • ✅ Time-block gym (completed)
  • ⏳ Anchor prayer to morning routine (in progress)
  • 📋 Create full day time-blocking (pending)

**Why this meeting now:** Weekly coaching check-in, review progress on morning routine

**Focus areas:** 
- Report on morning time-blocking success/challenges
- Expand time structure to afternoon
- Address any new overwhelm patterns

📎 Last session: https://app.fireflies.ai/view/01KJG0MFN6ZA43FFSV0DDRJYV5
```
